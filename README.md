...
## ✨ 주요 기능 및 로직 (Core Features & Logic)

`Seoro-town`은 학생들을 위한 다양한 편의 기능을 API로 제공하며, 웹과 모바일 앱에서 동일한 기능을 사용할 수 있습니다.

### 1. 사용자 인증 및 관리 (Authentication & Users)
- **로직**: JWT(JSON Web Token)를 사용한 토큰 기반 인증 시스템을 사용합니다.
- **기능**:
  - **회원가입**: 이메일 인증(SendGrid API 사용)을 거쳐야 가입이 완료됩니다.
  - **로그인**: 성공 시 JWT 토큰을 발급하여 클라이언트(localStorage)에 저장합니다.
  - **프로필 관리**: 마이페이지에서 프로필 이미지(Cloudinary에 업로드)와 닉네임 등을 수정할 수 있습니다.
  - **사용자 조회**: 다른 사용자의 프로필과 작성한 게시글을 조회할 수 있습니다.

### 2. 커뮤니티 게시판 (Posts)
- **로직**: `/api/posts` 엔드포인트를 통해 게시글에 대한 CRUD(생성, 조회, 수정, 삭제) 기능을 제공합니다.
- **기능**:
  - **게시글 작성/수정**: 이미지 첨부가 가능하며, 이미지는 Cloudinary에 저장됩니다.
  - **게시글 조회**: 전체 게시글 목록을 보거나, 특정 게시글을 상세 조회할 수 있습니다.
  - **검색**: 제목, 내용, 작성자 기준으로 게시글을 검색할 수 있습니다.

### 3. 중고 장터 (Market)
- **로직**: `/api/market` 엔드포인트를 통해 중고 거래 상품에 대한 CRUD 기능을 제공합니다.
- **기능**: 커뮤니티 게시판과 유사하게 판매 상품을 등록하고, 조회하며, 검색할 수 있습니다.

### 4. 실시간 채팅 (Real-time Chat)
- **로직**: HTTP API와 WebSocket(Socket.io)을 함께 사용하는 하이브리드 방식입니다.
- **기능**:
  - **채팅방 생성/조회**: `/api/chat` API를 통해 채팅방을 만들거나 참여 중인 채팅방 목록을 불러옵니다.
  - **실시간 메시지**: WebSocket에 연결된 후, 특정 방(room)에 `join`하여 실시간으로 메시지를 주고받습니다. (`sendMessage`, `receiveMessage` 이벤트)
  - **메시지 저장**: 주고받은 메시지는 MongoDB에 영구적으로 저장되어 언제든 다시 불러올 수 있습니다.

### 5. 가정통신문 크롤링 (School Announcements)
- **로직**: Puppeteer를 사용하여 실제 브라우저처럼 서울로봇고등학교 웹사이트에 접속하여 가정통신문 데이터를 직접 수집(크롤링)합니다.
- **기능**:
  - **자동 업데이트**: `/api/com` API 호출 시, 학교 웹사이트의 최신 가정통신문 목록과 상세 내용을 가져와 JSON 형태로 제공합니다.
  - **첨부파일 지원**: 원본 게시물의 첨부파일 다운로드 링크를 함께 제공합니다.
  - **캐싱**: 한 번 불러온 데이터는 일정 시간 동안 캐싱하여 다음 요청 시 더 빠른 속도로 응답합니다.

### 6. 급식 정보 (Meal Service)
- **로직**: `/api/meal` 엔드포인트를 통해 급식 정보를 제공합니다. (NEIS API 등을 활용할 것으로 추정)
- **기능**: 날짜별 급식 메뉴를 조회하여 앱과 웹에 표시합니다.
