...
const readFollows = async () => {
    try {
        ...
        console.log('follows.json 파일이 없어서 새로 생성합니다.');
        const emptyFollows = [];
        await writeFollows(emptyFollows);
        return emptyFollows;
    }
};
...
const writeFollows = async (follows) => {
    try {
        await fs.writeFile(FOLLOWS_FILE, JSON.stringify(follows, null, 2));
        console.log('팔로우 데이터 저장 완료');
    } catch (error) {
        console.error('팔로우 데이터 저장 오류:', error);
        throw error;
    }
};
...
app.get('/api/users/:id/posts', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            posts: userPosts.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        });
    } catch (error) {
...
app.get('/api/users/:id/comments', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            comments: userComments.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        });
    } catch (error) {
...
app.get('/api/users/:id/followers', authenticateToken, async (req, res) => {
    try {
        ...
        const followers = follows
            .filter(follow => follow.followingId === req.params.id)
            .map(follow => {
...
app.get('/api/users/:id/following', authenticateToken, async (req, res) => {
    try {
        ...
        const following = follows
            .filter(follow => follow.followerId === req.params.id)
            .map(follow => {
...
app.get('/api/users/:id/follow-status', authenticateToken, async (req, res) => {
    try {
        const follows = await readFollows();
        const isFollowing = follows.some(follow => 
            follow.followerId === req.user.id && follow.followingId === req.params.id
        );
...
app.post('/api/users/:id/follow', authenticateToken, async (req, res) => {
    try {
        ...
        const existingFollow = follows.find(follow => 
            follow.followerId === currentUserId && follow.followingId === targetUserId
        );
